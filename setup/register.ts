...
import { personalizeClaudeTemplate } from '../src/assistant-template.ts';
  ...
  for (let i = 0; i < args.length; i++) {
    switch (args[i]) {
      ...
      case '--is-main':
        ...
        break;
      case '--assistant-name':
        result.assistantName = args[++i] || 'NanoCures';
        break;
    }
  }
...
export async function run(args: string[]): Promise<void> {
  ...
  if (parsed.assistantName !== 'NanoCures') {
    logger.info(
      { from: 'NanoCures', to: parsed.assistantName },
    ...
    for (const mdFile of mdFiles) {
      if (fs.existsSync(mdFile)) {
        ...
        content = personalizeClaudeTemplate(content, parsed.assistantName);
