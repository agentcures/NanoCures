...
<dict>
    ...
    <string>${projectRoot}/logs/nanocures.log</string>
    <key>StandardErrorPath</key>
    <string>${projectRoot}/logs/nanocures.error.log</string>
</dict>
</plist>`;
  ...
  try {
    ...
    serviceLoaded = output.includes('com.nanocures');
  } catch {
...
/**
 * Kill any orphaned NanoCures node processes left from previous runs or debugging.
...
function killOrphanedProcesses(projectRoot: string): void {
  try {
    ...
    logger.info('Stopped any orphaned NanoCures processes');
  } catch {
...
function setupSystemd(
  ...
  if (runningAsRoot) {
    unitPath = '/etc/systemd/system/nanocures.service';
    ...
    unitPath = path.join(unitDir, 'nanocures.service');
    systemctlPrefix = 'systemctl --user';
  }

  const unit = `[Unit]
Description=NanoCures Disease Discovery Assistant
...
StandardOutput=append:${projectRoot}/logs/nanocures.log
StandardError=append:${projectRoot}/logs/nanocures.error.log
...
WantedBy=${runningAsRoot ? 'multi-user.target' : 'default.target'}`;
  ...
  // Kill orphaned NanoCures processes to avoid channel connection conflicts
  ...
  try {
    execSync(`${systemctlPrefix} enable nanocures`, { stdio: 'ignore' });
  } catch (err) {
    logger.error({ err }, 'systemctl enable failed');
  }

  try {
    execSync(`${systemctlPrefix} start nanocures`, { stdio: 'ignore' });
  } catch (err) {
  ...
  let serviceLoaded = false;
  try {
    execSync(`${systemctlPrefix} is-active nanocures`, { stdio: 'ignore' });
    serviceLoaded = true;
  } catch {
    // Not active
  }
...
function setupNohupFallback(
  ...
  const wrapperPath = path.join(projectRoot, 'start-nanocures.sh');
  const pidFile = path.join(projectRoot, 'nanocures.pid');

  const lines = [
    '#!/bin/bash',
    '# start-nanocures.sh — Start NanoCures without systemd',
    ...
    '    echo "Stopping existing NanoCures (PID $OLD_PID)..."',
    '    kill "$OLD_PID" 2>/dev/null || true',
    '    sleep 2',
    '  fi',
    'fi',
    '',
    'echo "Starting NanoCures..."',
    `nohup ${JSON.stringify(nodePath)} ${JSON.stringify(projectRoot + '/dist/index.js')} \\`,
    `  >> ${JSON.stringify(projectRoot + '/logs/nanocures.log')} \\`,
    `  2>> ${JSON.stringify(projectRoot + '/logs/nanocures.error.log')} &`,
    '',
    `echo $! > ${JSON.stringify(pidFile)}`,
    'echo "NanoCures started (PID $!)"',
    `echo "Logs: tail -f ${projectRoot}/logs/nanocures.log"`,
  ];
