...
describe('remote-control', () => {
  ...
  describe('startRemoteControl', () => {
    it('spawns claude remote-control and returns the URL', async () => {
      ...
      expect(spawnMock).toHaveBeenCalledWith(
        'claude',
        ['remote-control', '--name', 'NanoCures Remote'],
    ...
    it('returns existing URL if session is already active', async () => {
      ...
      // Second call by the same operator in the same chat should return the URL
      const result = await startRemoteControl('user1', 'tg:123', '/project');
      expect(result).toEqual({
        ok: true,
    ...
    it('denies access to an active session from another sender', async () => {
      const proc = createMockProcess();
      spawnMock.mockReturnValue(proc);
      stdoutFileContent = 'https://claude.ai/code?bridge=env_owner\n';
      vi.spyOn(process, 'kill').mockImplementation((() => true) as any);

      await startRemoteControl('user1', 'tg:123', '/project');

      const result = await startRemoteControl('user2', 'tg:123', '/project');
      expect(result).toEqual({
        ok: false,
        error: 'Remote Control session is owned by another operator.',
      });
      expect(spawnMock).toHaveBeenCalledTimes(1);
    });

    it('denies access to an active session from another chat', async () => {
      const proc = createMockProcess();
      spawnMock.mockReturnValue(proc);
      stdoutFileContent = 'https://claude.ai/code?bridge=env_chat\n';
      vi.spyOn(process, 'kill').mockImplementation((() => true) as any);

      await startRemoteControl('user1', 'tg:123', '/project');

      const result = await startRemoteControl('user1', 'tg:456', '/project');
      expect(result).toEqual({
        ok: false,
        error: 'Remote Control session is active in another chat.',
      });
      expect(spawnMock).toHaveBeenCalledTimes(1);
    });
  ...
  describe('stopRemoteControl', () => {
    ...
    it('denies stop requests from another sender', async () => {
      const proc = createMockProcess(55556);
      spawnMock.mockReturnValue(proc);
      stdoutFileContent = 'https://claude.ai/code?bridge=env_owner_stop\n';
      const killSpy = vi
        .spyOn(process, 'kill')
        .mockImplementation((() => true) as any);
      ...
      const result = stopRemoteControl('user2', 'tg:123');
      expect(result).toEqual({
        ok: false,
        error: 'Remote Control session is owned by another operator.',
      });
      expect(killSpy).not.toHaveBeenCalledWith(55556, 'SIGTERM');
  ...
  describe('restoreRemoteControl', () => {
    ...
    it('startRemoteControl returns restored URL without spawning', () => {
      ...
      return startRemoteControl('user1', 'tg:123', '/project').then(
        (result) => {
          expect(result).toEqual({
            ok: true,
            url: 'https://claude.ai/code?bridge=env_restored',
    ...
    it('denies restored session access to a different sender', () => {
      const session = {
        pid: 77777,
        url: 'https://claude.ai/code?bridge=env_restored',
        startedBy: 'user1',
      ...
      return startRemoteControl('user2', 'tg:123', '/project').then(
        (result) => {
          expect(result).toEqual({
            ok: false,
            error: 'Remote Control session is owned by another operator.',
