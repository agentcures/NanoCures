...
function getSessionAccessError(
  ...
  if (session.startedInChat !== chatJid) {
    return 'Remote Control session is active in another chat.';
  }
  if (session.startedBy !== sender) {
    return 'Remote Control session is owned by another operator.';
  }
  return null;
}
...
export async function startRemoteControl(
  ...
  if (activeSession) {
    ...
    if (isProcessAlive(activeSession.pid)) {
      const accessError = getSessionAccessError(activeSession, sender, chatJid);
      if (accessError) {
  ...
  // This prevents SIGPIPE when NanoCures restarts.
  ...
  let proc;
  try {
    proc = spawn('claude', ['remote-control', '--name', 'NanoCures Remote'], {
      cwd,
